...
다음 형식으로 정리해주세요. 각 섹션은 명확하게 구분하고, 해당 내용이 없으면 "없음"으로 작성하세요:

1. 증상:
(환자의 증상을 간결하게 정리)

2. 진단:
(의사의 진단명 또는 추정 진단만 간결하게 정리하여 텍스트로 기재 (진단명에 -, *같은 이모티콘 제거).
예: "급성 장염", "내분비내과 검사 필요", "감기 추정"
단, 정확한 진단명이 없을때는 "~추정, ~필요" 식으로 추정 진단명만 간결하게 기재)
